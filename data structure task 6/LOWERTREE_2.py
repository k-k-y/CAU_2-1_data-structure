...
while True:
    ...
    temp_arr.append([word, meaning])
f.close()

# temp_arr 배열에 미리 단어 정렬 해 놓기
temp_arr = sorted(temp_arr, key=lambda x: x[0].lower())

dicts = [AVLTree() for _ in range(26)]  # A ~ Z까지 알파벳에 따른 트리 객체 배열

index = 0
for i in range(26):
    while (index < len(temp_arr)):
        if (temp_arr[index][0][0] != chr(65 + i) and temp_arr[index][0][0] != chr(97 + i)):  # 단어의 첫 번째 문자를 비교
            break
        dicts[i].insert(temp_arr[index])
        index += 1

maximum = 0

for i in range(len(dicts)):
    if maximum < dicts[i].getTreeHeight(dicts[i].root):
        maximum = dicts[i].getTreeHeight(dicts[i].root)
print("사전의 높이 : " + str(maximum))

while True:
    word = input("단어를 입력하세요: ")
    if (word == ""):
        break
    print("뜻 : " + str(dicts[(ord(word[0].upper()) - 65)].search(word)))
print("Finish :)")
