...
void selection_sort(Student *arr, int n, char *check)
{
	int i, j, least;
	if (strcmp(check, "number") == 0)
	{
		for (i = 0; i < n - 1; i++)
		{
			...
			for (j = i + 1; j < n; j++)
			{
				if (strcmp((arr + least)->number, (arr + j)->number) > 0)
				{
					least = j;
				}
			}
	...
	else if (strcmp(check, "name") == 0)
	{
		for (i = 0; i < n - 1; i++)
		{
			...
			for (j = i + 1; j < n; j++)
			{
				if (strcmp((arr + least)->name, (arr + j)->name) > 0)
				{
					least = j;
				}
			}
...
int main(void)
{
	...
	// sort by number
	start = clock();
	selection_sort(copy_s, 20000, "number"); // sorting by number
	end = clock();
	...
	// sort by name
	start = clock();
	selection_sort(copy_s, 20000, "name"); // sorting by name
	end = clock();
